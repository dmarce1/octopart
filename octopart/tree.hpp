...
class tree: public hpx::components::migration_support<hpx::components::component_base<tree>> {
	...
	vector<gradient> grad_lim;
	vector<gradient> grad;
	vector<conserved_state> dudt;
	vector<vect> mass_flux;
	vector<real> Ncond;
	array<hpx::id_type, NCHILD> children;
	array<int, NCHILD> child_loads;
	vector<sibling_attr> siblings;
	hpx::id_type parent;
	hpx::id_type self;
	range root_box;
	range box;
...
public:
	tree();
	tree(vector<particle>&&, const range&, const range&);

	mass_attr compute_mass_attributes();
	void compute_drift(real);
	void compute_gradients();
	void compute_gravity(vector<hpx::id_type>, vector<mass_attr>);
	...
	vector<particle> destroy();
	void form_tree(vector<hpx::id_type>, bool = true);
	tree_attr finish_drift();
	tree_attr get_attributes() const;
	array<hpx::id_type, NCHILD> get_children() const;
	vector<gradient> get_gradients(range, range, const vect&) const;
	vector<gravity_part> get_gravity_particles() const;
	mass_attr get_mass_attributes() const;
	hpx::id_type get_parent() const;
	vector<vect> get_particle_positions(range, const vect&) const;
	vector<particle> get_particles(range, range, const vect&) const;
	void initialize(const std::string&);
	void redistribute_workload(int, int);
	void send_particles(const vector<particle>&, const vect&);
	...
	HPX_DEFINE_COMPONENT_ACTION(tree,get_gravity_particles);
	HPX_DEFINE_COMPONENT_ACTION(tree,get_mass_attributes);
